...
class DirectoryService():
    ...
    def notifyGroup(self,groupname,username,action,error=0):
        ...
        if groupname not in self.groups:
            ...
            return
        if action =="join":
            message = "joined:"+groupname+":"+username
        else:
            message = "left:"+groupname+":"+username
        conn=None
        users = ""
        ...
        if error==0:
            self.connections[conn].put(message)
        ...
        try:
            while True :
                ...
                if fields[0] == 'join':
                    ...
                    self.notifyGroup(groupname,username,"join")
                elif fields[0] == 'leave':
                    groupname = fields[1] + ":" + fields[2] #GROUP TO ENTER
                    username = fields[3] #USERNAME
        ...
        for key,value in self.groups.items():
            for f in value:
                if f[0] == connection:
                    ...
                    self.groups[groupname]= {item for item in self.groups[groupname] if item != value } 
...
if __name__ == "__main__":
    t = DirectoryService("0.0.0.0",8888,5)
    t.main()
    t.join()